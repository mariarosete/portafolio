...
const Experience: React.FC = () => (
  <section id="experiencia" className="max-w-[1440px] mx-auto px-3 sm:px-4 md:px-6 pt-10 pb-6">
          ...
          <article className="rounded-2xl border bg-white p-4 sm:p-5 transition hover:shadow-md hover:-translate-y-[1px]">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <h3 className="font-semibold text-[17px] sm:text-base">{x.company}</h3>
                <p className="text-slate-700 mt-1 leading-6">{x.role}</p>
                ...
                {Array.isArray(x.techs) && x.techs.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1.5 sm:gap-2">
                    {x.techs.map((t: string) => (
                      <span
                        key={t}
                        className="inline-flex items-center rounded-full px-2 py-1 text-[11px] sm:text-[12px] font-medium text-slate-700
                                   bg-gradient-to-r from-rose-50 to-amber-50 ring-1 ring-rose-200/50
              ...
              <span
                className="sm:ml-3 inline-flex self-start sm:self-auto
                           items-center rounded-full px-2.5 py-1 text-xs font-semibold
